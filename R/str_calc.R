...
freqSetting <- function(qData, rData, afInput, maf){
  ...
  for(i in 1:nL){
    ...
    afOneL <- afOneL[posAl]
    afAlOneL <- afAl[posAl]
    posUnobs <- which(is.element(alleleObs, afAlOneL) == FALSE)
    if(length(posUnobs) > 0){
      afAlOneL <- c(afAlOneL, alleleObs[posUnobs])
      afOneL <- c(afOneL, rep(maf, length(posUnobs)))
    }
...
calcApe <- function(afOneL){
  sigma1 <- sum(afOneL^2 * (1 - afOneL)^2)
  nAl <- length(afOneL)
  posAf <- combn(1:nAl, 2)
  sigma2 <- 0
  for(i in 1:ncol(posAf)){
    heteroAf <- afOneL[posAf[, i]]
