...
create_dt_criteria <- function(path_pack, init = TRUE){
  ...
  if(init && is.element("criteria.csv", fn_par)){
    ...
    dt_criteria <- data.table(Criteria = c("min_detect_auto", "min_lr_auto", "max_lr_auto", "min_detect_y", "max_mismatch_y", "max_mustep_y", "min_share_len_mt", "max_mismatch_mt"),
                              Value = c(0, 100, 0.01, 10, 4, 5, 300, 1))
