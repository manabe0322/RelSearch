...
load_server <- function(id, session_top, rv_criteria, rv_rel, rv_myu, rv_data_manage){
  moduleServer(
    ...
    function(input, output, session){
      ...
      observe({
        req(rv_criteria)
        rv_file$dt_criteria <- data.table(Criteria = c("min_detect_auto", "min_lr_auto", "max_lr_auto", "min_detect_y", "max_mismatch_y", "max_mustep_y", "min_share_len_mt", "max_mismatch_mt"),
      ...
      observe({
        ...
        rv_file$dt_rel <- data.table(Relationship = rv_rel$name,
                                     Sex_Victim = rv_rel$sex_v,
                                     Sex_Reference = rv_rel$sex_r,
                                     Pr_IBD2 = rv_rel$pibd2,
                                     Pr_IBD1 = rv_rel$pibd1,
                                     Pr_IBD0 = rv_rel$pibd0,
                                     Paternal = rv_rel$paternal,
                                     Maternal = rv_rel$maternal,
                                     Tree_persons = rv_rel$tree_persons,
                                     Tree_sexes = rv_rel$tree_sexes,
                                     Tree_fathers = rv_rel$tree_fathers,
                                     Tree_mothers = rv_rel$tree_mothers,
                                     Tree_founders = rv_rel$tree_founders)
      })
