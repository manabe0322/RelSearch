...
make_tab2 <- function(env_proj, env_gui){
  ...
  if(fin_auto){
    ...
    set_display_1 <- function(){
      ...
      select_min_lr_var <- tclVar(min_lr_auto)
    ...
    set_display_2 <- function(tf, select_q, select_r, select_rel, select_min_lr){
      ...
      if(select_q == "All"){
        ...
        pos_q <- which(sn_auto_q_vec == select_q)
      }
      if(select_r == "All"){
        pos_r <- 1:n_data
      }else{
        pos_r <- which(sn_auto_r_vec == select_r)
      }
      if(select_rel == "All"){
        pos_rel <- 1:n_data
      }else{
        pos_rel <- which(rel_auto_vec == select_rel)
      }
      pos_lr <- which(clr_all_vec > select_min_lr)
...
#    pd <- get("pd", pos = env_proj)
    ...
    data_all <- matrix(0, n_data, 4)
    colnames(data_all) <- c("Query", "Reference", "Relationship", "LR")
    data_all[, 1] <- sn_auto_q_vec
    data_all[, 2] <- sn_auto_r_vec
    data_all[, 3] <- rel_auto_vec
    data_all[, 4] <- sprintf('%.2e', clr_all_vec)

    # Define a matrix for the displayed data
    data_display <- data_all[which(clr_all_vec >= min_lr_auto), , drop = FALSE]
    data_display <- data_display[order(as.numeric(data_display[, 4]), decreasing = TRUE), , drop = FALSE]
