...
If full profiles are obtained for a victim-reference pair other than parent-child (i.e., without considering drop-out and mutations in calculation), the numerator of the LR is calculated using the probabilities that two, one and zero alleles are identity by descent (IBD).

$$
\begin{aligned}
Pr(G_{v}, G_{r}|H_{1}) &= Pr(G_{v}, G_{r}|IBD = 2)Pr(IBD = 2)
\\ &+ Pr(G_{v}, G_{r}|IBD = 1)Pr(IBD = 1)
\\ &+ Pr(G_{v}, G_{r}|IBD = 0)Pr(IBD = 0)
\end{aligned}
$$

In RelSearch, the $H_{2}$ is assumed to be unrelated between the victim and the reference; therefore, the denominator of the LR is simply calculated as follows:

$$
Pr(G_{v}, G_{r}|H_{2}) = Pr(G_{v})Pr(G_{r})
$$
