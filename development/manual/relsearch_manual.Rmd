...
```{r, echo = FALSE}
df_v_auto_ex <- data.frame(vic_prof = c("15", "15", "15", "-", "15", "15", "15, 16", "15, 16", "15, 17"), 
                           ...
                           ignore_do = c("", "", "", "Yes", "Yes", "", "", "", ""))
names(df_v_auto_ex) <- c("Victim profile", "Reference profile", "Mismatch", "Mutational steps", "Ignore due to allelic drop-out")

kable(df_v_auto_ex, format = "latex", booktabs = T, label = NA, linesep = "", escape = FALSE) %>%
  kable_styling(latex_options = "HOLD_position", font_size = 10)
```
